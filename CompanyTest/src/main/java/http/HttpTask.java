...
//        this.socket = socket;
    ...
    public void run() {
        try {
            ...
            //2.根据url在wepapp文件夹下去找是否存在这个资源,如果存在,就返回该文件夹的资源


            //改造:读取项目中的Login.html文件内容,并返回给客户端
            //怎么读取文件？ 1.绝对路径 2.相对路径怎么做?
            //相对路径读取步骤
            //(1).html文件所在的wabapp需要设置为resource资源文件夹----->将webapp中所有的文件复制到编译的输出文件夹
            //(2).通过getClassLoader().getResourceAsStream()获取文件的输入流(相对路径)
            InputStream is = this.getClass().getClassLoader().getResourceAsStream("." + request.getUrl()); //http请求行url以/开始
...
//                    response.addHeader("SESSIONID", sessionId);
                ...
                response.build200();
                response.println(content);
            }else{//以上所有路径都找不到,说明服务器不提供该url的服务,返回404.
                response.build404();
                response.println("找不到资源");
            }
        } catch (Exception e) { //捕获所有异常,表示服务器异常,需要返回500
            e.printStackTrace();
            response.build500();
