...
public class MinPrice {
    ...
    private static void init(){
        ...
        s = input.nextInt();  // 几种组合方式
        for(i=1; i<=s; i++){
            t = input.nextInt();  // 这个组合方式有几种
            for(j=1; j<=t; j++){
                n = input.nextInt(); // 编码
                p = input.nextInt(); //数量
                offer[i][num[n]] = p;   //把数量附给这个数组
            }
            offer[i][0] = input.nextInt();   //优惠价格
        }
    }
